...
back to current stat instead of stackstats : too long for big images.
threads for auto buttons 
avoid stackStatisticks in get_Sliders_Range for virtual stack 
add same contrast command
*/ 
import ij.*;
import ij.process.*;
import ij.gui.*;
...
public class Channels_and_Contrast implements PlugIn {
                ...
                new Thread(new Runnable() {
                    public void run() {
                        ...
                        for (int channel = 1; channel <= nChannels; channel++) {
                            IJ.showProgress((double) (channel / nChannels));
                                ...
                                for (int k = 0; k < icons_List.size(); k++) {
                                    if (icons_List.get(k).getBounds().contains(event.getPoint())) {
                                        if (SwingUtilities.isLeftMouseButton(event) && pressed_Index == k) {
                                            ...
                                            if (imp.isComposite()) ((CompositeImage)current_Imp).setDisplayMode(IJ.COMPOSITE);
                                            imp.updateAndDraw();
    ...
    void get_Sliders_Range(){
        ...
        if (bit_Depth == 8) {
            ...
            if (current_Imp.getStack().isVirtual()) stats = current_Imp.getStatistics();
            else stats = new StackStatistics(current_Imp);
            slider_Min = 0;
            slider_Max = 65535;
