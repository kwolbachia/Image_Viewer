...
function split_View_Dialog(){
	...
	Dialog.addNumber("Font size", round(Image.height()/9));
	Dialog.show();
...
function split_View(MONTAGE_STYLE, COLOR_MODE, LABELS) {
	...
	function square_SplitView(){
		...
		if (channels == 4)	{
			...
			combine_Vertically(channel_1_2_Overlay, channel_3_4);
		}
		if (channels == 5){
			channel_1_2_3 = combine_Horizontally(channel_1_2, TILES[3]); 	
			channel_4_5 =	combine_Horizontally(TILES[4], TILES[5]); 
			channel_4_5_Overlay =	combine_Horizontally(channel_4_5, TILES[0]); 
			combine_Vertically(channel_1_2_3, channel_4_5_Overlay);
		}
	}
