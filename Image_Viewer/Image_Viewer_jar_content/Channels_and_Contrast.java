...
public class Channels_and_Contrast implements PlugIn {
    ...
    boolean check_Active_Image() {
        ...
        if (imp != null) {
            if (imp.isLocked() || imp.isLockedByAnotherThread()){
                UPDATING = true;
                if (debug) IJ.log("check_Active_Image : imp.isLocked()=" + imp.isLocked() + " LockedByAnotherThread()=" + imp.isLockedByAnotherThread());
                return false;
            }
            else if (!imp.isVisible()){
                UPDATING = true;
                if (debug) IJ.log("check_Active_Image : imp.isVisible()=" + imp.isVisible());
                return false;
            }
            else UPDATING = false;
        }
        else UPDATING = false;
        boolean no_Image, is_RGB, is_weird_Stack, is_more_than_8_channels;
        no_Image = (imp == null);
        is_RGB = imp == null ? false : (imp.getBitDepth() == 24);
        is_weird_Stack = imp == null ? false : (imp.getNChannels()>1 && !imp.isComposite());
        is_more_than_8_channels = imp == null ? false : (imp.getNChannels()>8);

        if (no_Image || is_RGB || is_weird_Stack || is_more_than_8_channels) {
            ...
            JPanel idle_Panel = new JPanel();
            JButton bc_button = new JButton("Open B&C");
            bc_button.addActionListener(e -> IJ.run("Brightness/Contrast..."));
            idle_Panel.add(new JLabel("No image or type not supported "));
            idle_Panel.add(bc_button);
            MAIN_PANEL.add(idle_Panel);
            MAIN_PANEL.revalidate();
            MAIN_PANEL.repaint();
            PLUGIN_LOCKED = true;
            LAST_LUTS = null;
            addKeyListenerRecursively(FRAME, IJ.getInstance());
            return false;
        }
        if (imp != null && imp != IMP) {
            long now = System.currentTimeMillis();
    ...
    void save_Current_State() {
        ...
        LAST_PROJECTION_TYPE = IMP.getProp("CompositeProjection");
    ...
    boolean something_Changed() {
        ...
        // Composite Projection mode
        if (IMP.getProp("CompositeProjection") != LAST_PROJECTION_TYPE) {
            something_Changed = true;
            whats_detected += " Composite Projection mode changed";
        }
        // current channel
    ...
    void update_UI(){
        ...
        // Composite buttons
            ...
            String composite_Label = "Composite";
            String projection_Mode = IMP.getProp("CompositeProjection");
            if(projection_Mode == "Max") composite_Label = "Composite Max";
            if(projection_Mode == "Min") composite_Label = "Composite Min";
            if(projection_Mode == "Invert") composite_Label = "Composite Invert";
            COMPOSITE_BUTTON.setText(composite_Label);
        }
        UPDATING = false;
        save_Current_State();
    }
    ...
    JPanel get_Display_Mode_Panel() {
        ...
        String composite_Label = "Composite";
        String mode = IMP.getProp("CompositeProjection");
        if(mode == "Max") composite_Label = "Composite Max";
        if(mode == "Min") composite_Label = "Composite Min";
        ...
        COMPOSITE_BUTTON = new JToggleButton(composite_Label){
            ...
            protected void paintComponent(Graphics g) { // vertical 3 dots
                ...
                int cy = getHeight() / 2;
                int dotSize = 2;
                g2.fillOval(x, cy - 4, dotSize, dotSize);
                g2.fillOval(x, cy    , dotSize, dotSize);
                g2.fillOval(x, cy + 4, dotSize, dotSize);
            }
        };
        COMPOSITE_BUTTON.setToolTipText("Displays all channels as a composite overlay\nRight click to change mode");
        ...
        COMPOSITE_BUTTON.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                if (SwingUtilities.isRightMouseButton(e)) {
                    JPopupMenu popup = new JPopupMenu();
                    JMenuItem sum_mode = new JMenuItem("Composite Sum (default)");
                    sum_mode.addActionListener(event -> {
                        IMP.setProp("CompositeProjection", "Sum");
                        IMP.setDisplayMode(IJ.COMPOSITE);
                    });
                    popup.add(sum_mode);
                    JMenuItem max_mode = new JMenuItem("Composite Max");
                    max_mode.addActionListener(event -> {
                        IMP.setProp("CompositeProjection", "Max");
                        IMP.setDisplayMode(IJ.COMPOSITE);
                    });
                    popup.add(max_mode);
                    JMenuItem min_mode = new JMenuItem("Composite Min");
                    min_mode.addActionListener(event -> {
                        IMP.setProp("CompositeProjection", "Min");
                        IMP.setDisplayMode(IJ.COMPOSITE);
                    });
                    popup.add(min_mode);
                    JMenuItem invert_mode = new JMenuItem("Composite Invert");
                    invert_mode.addActionListener(event -> {
                        IMP.setProp("CompositeProjection", "Invert");
                        IMP.setDisplayMode(IJ.COMPOSITE);
                    });
                    popup.add(invert_mode);
                    popup.show(COMPOSITE_BUTTON, 5, COMPOSITE_BUTTON.getHeight());
                } else {
        ...
        fav_LUTs_Button.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                JPopupMenu popup = new JPopupMenu();
                    ...
                    JPanel invert = new JPanel(new FlowLayout(FlowLayout.CENTER, 2, 2));
                    invert.add(new JLabel("Invert LUT", JLabel.CENTER));
    ...
    private void addKeyListenerRecursively(Component component, KeyListener listener) {
